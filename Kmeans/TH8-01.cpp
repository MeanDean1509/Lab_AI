...
double tinhKhoangCachEuclidean(Toado diem, Toado trungTam) {
    return sqrt(pow(diem.x - trungTam.x, 2) + pow(diem.y - trungTam.y, 2));
}
...
int ganVaoCum(Toado diem, Toado* cum, int k) {
    ...
    for (int i = 0; i < k; i++) {
        double khoangCach = tinhKhoangCachEuclidean(diem, cum[i]);
